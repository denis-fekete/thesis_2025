...
\chapter{Object detection process}
...
For example, the camera provides an image with\texttt{1080x1920} resolution in portrait mode. The model requires an image with a resolution of \texttt{640x640}. Rescaling the image would create a \texttt{640x640} image; however, the original aspect ratio of the image \texttt{9:16}. This would be changed to a \texttt{1:1} aspect ratio, and the distortion could affect object detection.

Instead, if the image is resized using the larger dimension. Both dimensions will be scaled by the same scale factor, resulting in an image with a \texttt{640x360} resolution. However, this results in only one of the dimensions satisfying the resolution required by the model. \textbf{Letter-boxing} fixes this problem by padding the image with a value/color in one of the dimensions. This will result in a final image not being distorted and in the correct \texttt{640x640} resolution.
...
More popular and widely used inference runtimes are TensorFlow Lite and Open Neural Network Exchange Runtime (hereinafter referred to as ONNX and ORT for the runtime). Both projects are backed by leading companies in the computer vision space, such as Google and Microsoft, respectively. Both runtimes also support mobile devices. A study from 2023 \cite{Ren2023_ONNX_TensorFlow_BScan} shows ONNX out-performing TensorFlow on both CPU and GPU. The other blog post \cite{aimtechnoOnnxTFcomparison} from 2025 shows the gap closing as the performance difference between the runtimes is decreasing, with both having their pros and cons. The study also points out the open nature of the ONNX Runtime and its more multi-platform development approach, whereas TensorFlow Lite can benefit from Google's ecosystem. The ONNX Runtime was chosen as the main inference runtime. However, since the \textbf{CVLiBG} was designed to be expandable and modular, adding another runtime should be an easy task for developers interested in different approaches.
